...
export function exportArtboards(prefs) {
  ...
  console.log(
    `"${bundlePath}/Contents/Resources/sketchtool/bin/sketchtool" export artboards "${currentFileName}" --formats="${exportFormat ||
      "png"}" --scales="${exportScale}" --output="${fileFolder}" --overwriting=YES --items="${artboards.join(
      ","
    )}" --include-symbols=YES`
  );
  exec(
